...
continues["Description"] = ["Total value of trade between reporting and partner countries",
                            ...
                            "Foreign direct investment inflows for between reporting and partner countries",
                            "Foreign direct investment outflows for between reporting and partner countries"]
...
if visualise_all:
    ...
    sns.pairplot(data_numeric, vars=["FLOW_IN", "distw", "gdp_o"])  # kind="reg"/kind="kde"
    print(data['FLOW_IN'].describe())

    flows_winsorized = mstats.winsorize(data['FLOW_IN'], limits=[0.05, 0.05])
    layout = go.Layout(
        title="Basic histogram of FDI inflows (winsorized)")
    ...
    sns.distplot(data['FLOW_IN'], axlabel="Basic histogram of flows", color="y")
...
hist_all = sns.distplot(data["FLOW_IN"], axlabel="FDI of inflows", color="blue")


hist_all.figure.savefig('Histogram of FDI inflows over the history.png', bbox_inches="tight")

# Histograms for chosen years
years = (1994, 2000, 2009, 2012)
for i in years:
    plt.figure(i)
    hist_temp = sns.distplot(data["FLOW_IN"].loc[data['yr'] == i], axlabel="Logarithm of FDI flows in year " + str(i), color="blue")
    hist_temp.figure.savefig('Histogram of FDI inflows for ' + str(i) + '.png', bbox_inches="tight")
...
data_pairplot = data_numeric[["FLOW_IN", "distw", "gdp_d"]]
...
pairplot = sns.pairplot(data_pairplot, vars=["FLOW_IN", "distw", "gdp_d"], kind="scatter", markers=".",  diag_kind="kde",
...
flows = data[['yr', 'rt3ISO', 'pt3ISO', 'FLOW_IN']]
...
for i in range(len(flows)):
    if (flows['FLOW_IN'][i] > flow_treshold):
...
layout = dict(
        title='FDI inflows between Poland and its partners.',
        showlegend=False,
        geo=dict(
            scope=scope,
...
x_train = data_PL.drop('yr', axis=1).drop('FLOW_IN', axis=1).loc[data_PL['yr'] <= splitting_yr].values
y_train = data_PL.loc[:, 'FLOW_IN'].loc[data_PL['yr'] <= splitting_yr].values
x_test = data_PL.drop('yr', axis=1).drop('FLOW_IN', axis=1).loc[data_PL['yr'] > splitting_yr].values
y_test = data_PL.loc[:, 'FLOW_IN'].loc[data_PL['yr'] > splitting_yr].values
